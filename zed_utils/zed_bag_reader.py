...
class ZEDBagReader:
    ...
    def _handle_rgb(self, msg):
        ...
        total_times = self.extract_sensor_time(
            msg
        )  # You can store or use this timestamp as needed
    ...
    def _handle_pc(self, msg):
        ...
        x = np.ndarray(
            shape=(num_points,),
            dtype=get_dtype(fields["x"].datatype),
            buffer=buf,
        ...
        y = np.ndarray(
            shape=(num_points,),
            dtype=get_dtype(fields["y"].datatype),
            buffer=buf,
        ...
        z = np.ndarray(
            shape=(num_points,),
            dtype=get_dtype(fields["z"].datatype),
            buffer=buf,
    ...
    @property
    def poses(self):
        return self._synced_poses

    def _synchronize_poses(self):
        if not self._images or not self._poses:
            self._synced_poses = []
            return

        image_timestamps = [ts for ts, _ in self._images]
        pose_timestamps = [ts for ts, _, _ in self._poses]

        self._synced_poses = []
        for img_ts in image_timestamps:
            idx = bisect.bisect_right(pose_timestamps, img_ts) - 1
            if idx < 0:
                idx = 0

            ts, pos, quat = self._poses[idx]
            self._synced_poses.append((img_ts, pos, quat))

    @property
    def synced_poses(self):
        return self._synced_poses
